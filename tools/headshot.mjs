...
const ctx = await b.newContext({ viewport: { width: 460, height: 560 }, deviceScaleFactor: 1.6, serviceWorkers: 'block' })
...
const poses = process.argv[3] ? process.argv[3].split(',') : ['sit','idle','sleep','groom','eat']
for (const pose of poses) {
  await p.evaluate((b) => window.__catScene.freeze(b), pose)
  await p.waitForTimeout(9000)
  await p.evaluate(() => { window.__catScene.setCamera(0.0, 0.10, 0.85); window.__catScene.setHour(11) })
  await p.waitForTimeout(900)
  await p.screenshot({ path: `${out}/h-${pose}-frente.png` })
  await p.evaluate(() => window.__catScene.setCamera(1.5, 0.12, 0.95))
  await p.waitForTimeout(900)
  await p.screenshot({ path: `${out}/h-${pose}-lado.png` })
}
console.log('ok')
await b.close()
